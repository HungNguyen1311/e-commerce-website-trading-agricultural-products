        ...
        <section class="panel">
            ...
            <div class="panel-body">
                ...
                <div class="table-agile-info">
                    ...
                    @endforeach
                    <h3 class="text-right mt-3">Tổng giá trị đơn hàng:<span class="text-danger">{{number_format($edit_order_customer->tongtien)}}<u>đ</u></span></h3>
                    <form role="form" action="{{URL::to('/update-order-product/'.$edit_order_customer->iddonhang)}}" method="POST">
                    {{csrf_field()}}
                        <input type="hidden" name="staff_id_hidden" value="{{Session::get('admin_id')}}">
                        <select class="form-select order-select-status" aria-label="Default select example" name="order_status">
                        @if($edit_order_customer->trangthai=='1')
                        ...
                        @elseif($edit_order_customer->trangthai=='0')
                                <option selected value="0">Đang chờ xử lý</option>
                                <option value="1">Đã xác nhận</option>
                                <option value="2">Đang chuẩn bị hàng</option>
                                <option value="3">Đơn hàng đã xử lý</option>
                        @elseif($edit_order_customer->trangthai=='2')
